  ...
  <div class>
    <div class="container">
      ...
      <div class="d-flex pt-4 pb-4 align-items-center text-center">
        ...
        <div
          class="d-flex align-middle ml-auto text-center justify-content-center"
        >
          <div class="download">
        ...
        <table class="table table-bordered text-center table-hover table-fixed">
          ...
          <thead>
            <tr style="font-weight:bold; font-size:24px;">
              <th
                ...
                scope="col"
                :colspan="
                  tempData[0].queryResult.length * calcResult(resultForamt) + 7
                "
              >
                人工智慧即時溫度監控簽核表單
              </th>
          ...
          <thead>
            ...
            <tr>
              <th
                scope="col"
                v-for="(n, index) in tempData[0].queryResult.length *
                  calcResult(resultForamt)"
                :key="index"
              >
                <span v-if="index % 3 == 1 || calcResult(resultForamt) == 1"
                  >Ave</span
                >
                <span v-if="index % 3 == 2 && calcResult(resultForamt) == 3"
                  >Min</span
                >
                <span v-if="index % 3 == 0 && calcResult(resultForamt) == 3"
                  >Max</span
                >
              </th>
            </tr>
...
export default {
  ...
  created() {
    ...
    console.log(currDateFormat, day, diff, firstDay, lastDate);
        ...
        if (!el.disabled && el.checked) {
          ...
          for (let nodeIndex = 0; nodeIndex < vm.tempData.length; nodeIndex++) {
            if (vm.tempData[nodeIndex].node === nodeName) {
              for (
                let i = 0;
                i < vm.tempData[nodeIndex].queryResult.length;
                i++
              ) {
                vm.tempData[nodeIndex].queryResult[i].status = "signed";
                vm.tempData[nodeIndex].queryResult[i].statusOwner =
                  vm.userInfo.name;
